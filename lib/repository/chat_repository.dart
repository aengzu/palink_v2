...
class ChatRepository {
  ...
  Future<Message?> sendAIResponseAsMessage(AIResponse aiResponse, int conversationId) async {
    MessageDto messageDto = MessageUtils.convertAIMessageToMessageDto(aiResponse, conversationId);
  ...
  Future<Likability> sendLikingLevel(String userId, int characterId, int likingLevel, int messageId) async {
    return await _chatService.sendLikingLevel(userId, characterId, likingLevel, messageId);
  }
  ...
  Future<Message?> sendUserMessage(String text, int conversationId) async {
    MessageDto messageDto = MessageUtils.createUserMessageDto(text, conversationId);
    return await sendMessage(messageDto);
  }
}
