...
import 'package:palink_v2/controllers/user_controller.dart';
...
import '../views/chatting/conversation_end_loading.dart';
import 'conversation_end_controller.dart';
...
class ChatViewModel extends GetxController {
  ...
  Future<void> loadMessages() async {
    ...
    try {
      List<Message> list =
          await chatRepository.getMessagesByChatRoomId(chatRoomId);
      messages.value = list;
    } catch (e) {
  ...
  Future<void> sendMessage() async {
    ...
    try {
      // 사용자 메시지
      Message? sentUserMessage =
          await chatRepository.sendUserMessage(text, chatRoomId);
      if (sentUserMessage != null) {
        ...
        likingLevels.insert(
            ...
            LikingLevel(
                likingLevel: 0,
                messageId: sentUserMessage.messageId)); // 더미 호감도 저장
      }

      // AI 메시지
      AIResponse? aiResponse =
          await openAIService.invokeChain(text); // ai로 text 넣어서 응답받는거
      if (aiResponse != null) {
        MessageDto botMessageDto =
            MessageUtils.convertAIMessageToMessageDto(aiResponse, chatRoomId);
        Message? sentBotMessage =
            await chatRepository.sendMessage(botMessageDto);

        // 여기에 aiResponse의 isEnd 값확인하고, 1이면 종료
        // 대화 종료 체크
        if (aiResponse.isEnd == 1) {
          print('호감도: ${aiResponse.affinityScore}');
          ConversationEndController conversationEndController =
              ConversationEndController(
                  character, messages, chatRoomId, aiResponse.affinityScore);
          Get.to(() => ConversationEndLoadingView(
        ...
        if (sentBotMessage != null) {
          ...
          backgroundColor.value =
              emotionVibration.getColorForEmotion(aiResponse.expectedEmotion);
          // AI의 감정에 따른 진동 실행
          ...
          chatRepository.sendLikingLevel(
              ...
              aiResponse.affinityScore,
              sentBotMessage.messageId);
          // 팁 업데이트
