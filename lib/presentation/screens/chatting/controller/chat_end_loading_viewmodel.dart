...
class ChatEndLoadingViewModel extends GetxController {
  ...
  Future<void> _analyzeConversation(Character character) async {
    try {
      List<AIResponseResponse> chatHistory = await getAIMessagesUsecase.execute(conversationId);
      ...
      final String chatHistoryString = chatHistory.map((response) {
        ...
        final List<String> rejectionContent = List<String>.from(response.rejectionContent);
        return '''
